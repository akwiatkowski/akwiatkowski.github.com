...
class Commands::AssignPhotosToAreas
  ENVS = ["dev", "full"]

  AREA_TYPES = [
    {name: "towns", getter: :towns},
    {name: "counties", getter: :counties},
    {name: "voivodeships", getter: :voivodeships},
    {name: "meso_regions", getter: :meso_regions},
    {name: "macro_regions", getter: :macro_regions},
  ]
  ...
  private def process_env(env : String)
    ...
    AREA_TYPES.each do |at|
      subdir = File.join([cache_dir, at[:name]])
    ...
    # Accumulate results: {area_type_name, slug} => [{filename, post_slug}]
    ...
    photos_to_process.each_with_index do |(photo_id, lat, lon), idx|
      ...
      # Map PointMatchResult fields to area type names
    ...
    # Create empty files for areas that don't have any photos yet
    ensure_empty_files(cache_dir)
  ...
  private def read_manifest(path : String) : Set(String)
    return Set(String).new unless File.exists?(path)
    lines = File.read(path).split("\n").reject(&.empty?)
    lines.to_set
  end

  private def read_cache_file(path : String) : Array({String, String})
  ...
  private def write_cache_file(path : String, entries : Array({String, String}))
  ...
  private def ensure_empty_files(cache_dir : String)
    # Create empty YAML files for all known areas that don't have cache files yet
...
command = Commands::AssignPhotosToAreas.new(overwrite: overwrite)
command.run
