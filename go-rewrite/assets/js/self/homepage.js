// Homepage dynamic content (Go version)
...
//
// Adapted for Go JSON structure:
// - No visible/ready filter (all posts are finished)
// - Tags: json.tags[slug] map lookup
// - Areas: json.areas.meso_regions[slug] map lookup
// - Photos: post.top_photos[].url/.avif
(function() {
  'use strict';
  ...
  function getPostMonth(post) {
    return new Date(post.time || post.date).getMonth() + 1;
  }

  function getPostTime(post) {
    return post.time || post.date;
  }
  ...
  // Check if post tags array (string slugs) contains a specific slug
  function hasTagSlug(tags, slug) {
    if (!tags || !Array.isArray(tags)) return false;
    return tags.indexOf(slug) >= 0;
  }

  function selectHeroPost(posts, tagsMap) {
    var currentMonth = getCurrentMonth();
    var currentHour = getCurrentHour();
    var eligiblePosts = posts.filter(function(p) {
      return p.tags && p.tags.length > 0 && p.top_photos && p.top_photos.length > 0;
    });
    ...
    var scored = eligiblePosts.map(function(post) {
      ...
      var postTime = getPostTime(post);
      var postYear = new Date(postTime).getFullYear();
  ...
  // Select a random photo from the post's top_photos array
  // Uses weighted selection favoring higher-scored photos
  function selectHeroPhoto(post) {
    if (!post.top_photos || post.top_photos.length === 0) {
      return { url: post.card_image_url, alt: post.title };
    }

    var photos = post.top_photos;
    var weights = photos.map(function(p) {
      return Math.max(1, p.points + 1);
    });
  ...
  function selectPosts(posts, heroPost) {
    ...
    var byDate = eligiblePosts.slice().sort(function(a, b) {
      return new Date(getPostTime(b)) - new Date(getPostTime(a));
    });
  ...
  function selectCategoryChips(posts, tagsMap, areasMap) {
    var chips = [];
    var usedSlugs = {};
    var mesoRegions = (areasMap && areasMap.meso_regions) || {};

    // Add fixed chips - lookup from tags map
    FIXED_CHIPS.forEach(function(chip) {
      var tag = tagsMap[chip.slug];
      if (tag) {
  ...
  // Get primary tag name using tags map
  function getPrimaryTagName(postTags, tagsMap) {
    if (!postTags || postTags.length === 0) return 'Wpis';

    var priority = ['best', 'najlepsze', 'bicycle', 'hike', 'train'];
    for (var i = 0; i < priority.length; i++) {
      if (hasTagSlug(postTags, priority[i]) && tagsMap[priority[i]]) {
        return tagsMap[priority[i]].name;
      }
    }

    // Return first tag's name from map
    var firstTag = tagsMap[postTags[0]];
    return (firstTag && firstTag.name) || 'Wpis';
  }
  ...
  function createPostCardElement(post, isFeatured, tagsMap) {
    var card = document.createElement('a');
    card.href = post.url;
    card.className = 'post-card' + (isFeatured ? ' post-featured' : '');

    var primaryTag = getPrimaryTagName(post.tags, tagsMap);
    ...
    var dateStr = formatDatePolish(getPostTime(post));
  ...
  function init() {
    fetch(CONFIG.jsonUrl)
      ...
      .then(function(json) {
        ...
        var tagsMap = json.tags || {};
        var areasMap = json.areas || {};
        ...
        var heroPost = selectHeroPost(posts, tagsMap);
        var gridPosts = selectPosts(posts, heroPost);
        var chips = selectCategoryChips(posts, tagsMap, areasMap);

        // Update hero image - select random photo from post's top_photos
        ...
        if (heroLink && heroPost) {
          ...
          if (heroImg) {
            heroImg.src = heroPhoto.url || heroPost.card_image_url;
            heroImg.alt = heroPost.title;
        ...
        if (postsContainer) {
          ...
          gridPosts.forEach(function(post, index) {
            var card = createPostCardElement(post, index === 0, tagsMap);
