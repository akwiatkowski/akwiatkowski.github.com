...
func TestMaxMtime(t *testing.T) {
	...
	maxTime, err := MaxMtime(f1, f2)
	if err != nil {
		t.Fatal(err)
	}

	info2, _ := os.Stat(f2)
	if !maxTime.Equal(info2.ModTime()) {
...
func TestMaxMtimeSkipsNonExistent(t *testing.T) {
	...
	maxTime, err := MaxMtime(f1, filepath.Join(dir, "nonexistent.txt"))
	if err != nil {
		t.Fatal(err)
	}

	info, _ := os.Stat(f1)
	if !maxTime.Equal(info.ModTime()) {
		t.Errorf("expected max to be a.txt mtime")
	}
}

func TestMaxMtimeAllNonExistent(t *testing.T) {
	maxTime, err := MaxMtime("/nonexistent/a", "/nonexistent/b")
	if err != nil {
		t.Fatal(err)
	}
	if !maxTime.IsZero() {
		t.Errorf("expected zero time, got %v", maxTime)
	}
}
...
func TestFileNewerThan(t *testing.T) {
	...
	os.WriteFile(source, []byte("s"), 0o644)
...
func TestAnyNewerThan(t *testing.T) {
	...
	os.WriteFile(fresh, []byte("f"), 0o644)
