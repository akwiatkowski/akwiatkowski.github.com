...
//
// When force=false, if the polygon output directory already has content,
// the expensive 93MB external YAML load is skipped entirely. This means
// incremental builds may miss newly visited areas — use --force for a
// full re-render before deploying.
func Generate(externalDir, outputDir, areaCacheDir string, posts []*model.Post, areas []*model.Area, tolerance float64, force bool) (GenerateResult, error) {
	...
	// Quick check: if polygon output dir has content and force=false,
	// skip the expensive 93MB external YAML load entirely.
	// Incremental builds accept potentially stale polygons — use --force
	// for full re-renders before uploading to production.
	if !force && polygonDirHasContent(outputDir) {
		return result, nil
	}
...
// polygonDirHasContent checks if the polygon output directory exists and
// contains at least one subdirectory with files. Used to skip the expensive
// external YAML load on incremental builds.
func polygonDirHasContent(outputDir string) bool {
	entries, err := os.ReadDir(outputDir)
	if err != nil || len(entries) == 0 {
		return false
	}
	// Check that at least one type subdirectory has polygon files
	for _, entry := range entries {
		if !entry.IsDir() {
			continue
		}
		subEntries, err := os.ReadDir(filepath.Join(outputDir, entry.Name()))
