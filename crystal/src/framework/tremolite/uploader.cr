...
class Tremolite::Uploader
  ...
  def upload(f : String)
    ...
    Log.info { "#{f} upload size #{File.size(f)}" }

    remote_time = check_on_ftp(f)
    Log.info { "#{f} post upload REMOTE #{remote_time}" }
  end

  def upload_if_needed(f : String)
    # we only check for real files
    remote_time = check_on_ftp(f)
    Log.info { "#{f} checked REMOTE #{remote_time}" }

    if remote_time
      local_time = File.lstat(f).mtime

      if local_time > remote_time
        Log.info { "#{f} LOCAL is newer #{local_time}" }
        return upload(f)
      else
        Log.debug { "#{f} LOCAL is not newer" }
      end
    else
      return upload(f)
    end
