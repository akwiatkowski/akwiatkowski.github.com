...
## Phase 19: New Index Page ✅ IN PROGRESS

### Overview

New modern index page with dynamic content selection. Implemented as `NewHomePageView` at `/index.html`. Old index moved to `/index.old.html`.
...
- [x] Create optimized JSON generator (`HomePageJsonGenerator` → `/jsons/homepage.json`)
- [x] Include top 4 photos per post (by points) in JSON
- [x] Write hero photo selection JS (weighted random from post's photos)
- [x] Write post mixing JS (latest, seasonal, throwback)
- [x] Write chip selection JS (fixed + dynamic regions + discovery tags)
- [x] Create index page view (`NewHomePageView` at `/index.html`)
- [x] Light/dark mode via CSS variables
- [x] Responsive design
- [x] Replace index.html, move old to `/index.old.html`
- [x] Tag URLs: `/tag/` → `/wpisy-dla/tagu/` with 302 redirects
- [x] Navigation URLs generated by Router (not hardcoded)
- [x] E2E tests for homepage links
- [ ] Add `finished_at` to payload.json posts (deferred)
