...
południowe okolice [Malborka][wiki-malbork] i chciałem z niego wyruszyć na
południe.

Patrząc na połączenia kolejowe nie miałem dobrego nastroju. Większość to
pendolino (IC Premium) które kosztuje 70zł + rower. Jedyne połączenia osobowe
...
Gdy budzik się odezwał o 4:45 słyszałem go we śnie i nie zrozumiałem co to jest.
...
Spotkałem dwóch rowerzystów, którzy jadą do [Elbląga][wiki-elblag] a następnie
"gdzieś na wschód".
...
się bezpośrednio do [Sztumu][wiki-sztum]. Przywitały nas ładne nieduże górki.

TODO zdjęcia Jeziora Dąbrówka, obok wiatraki

Przejechaliśmy przez wieś [Koślinka][wiki-koslinka]. Ruch o 7-ej rano w święta
był praktycznie zerowy. Według GPSu Macieja rano temperatura wynosiła około 4
stopni.

Jechaliśmy znacznie spokojniej. Nie planowaliśmy 100km ale nie przekreślaliśmy tego.
...
Następnie jadąc wzdłuż Jeziora Sztumskiego objechaliśmy kościół i zamek krzyżacki.
TODO wiki sprawdzić. Mijaliśmy dużo rybaków z wędkami.
...
W okolicy [Postolina][wiki-postolin] okolica stała się wyraźnie klimatyczna.
Jeżeli się nie mylę, to sama wioska była bardzo ładna. Jest tam ciekawy kościół TODO.
...
nie śpią tylko tutaj, ale żyją, pracują. Gdzie domy nie są budowane zgodnie
z jednym, uniwersalnym i nowoczesnym projektem.
...
położoną znacznie wyżej niż wschodia część wsi.
...
Z wysokości 75m zjechaliśmy do 50m (około), do doliny rzeki [Liwa][wiki-liwa-rzeka].
Sama [rzeka][wiki-liwa-rzeka] w pobliżu mostu, którym przejechaliśmy, była
dość wartka.

Droga przez las nie była taka zła - głównie szutrowa.
...
Mnie samego by interesowała ta trasa, zwłaszcza okolica mostu nad [Wisłą][wiki-wisla].
...
jak pojedziemy chwilę na wschód i ruszymy do [Otoczyna][wiki-otoczyn].
Różnica wynosiła półtora kilometra ale dzięki temu jechaliśmy przez
...
Patrząc teraz na mapę to ciekawią mnie jeziora w okolicy wsi [Klasztore][wiki-klasztorek],
położone na wschód.
...
takich miejsc ale nie lubię, gdy nie wiem czy teren jest prywatny czy publiczny.

Za domem była droga gruntowa, dość zarośnięta, to właśnie nią musieliśmy
jechać aby dostać się do wsi [Nogat][wiki-nogat].

TODO tunel w krzewach

Jadąc w okolicy [Starego Błonowa][wiki-stare-blonowo] mieliśmy
bardzo dobry widok na obniżenie terenu po lewej. Im bliżej byliśmy
do [Łasina][wiki-lasin] tym różnica wysokości terenu była mniejsza.

Sporo tego dnia nagrywałem na kamerce i musiałem teraz oszczędzać
...
zamknięta. Ktoś nam polecił jakiś bar. Ja zamówiłem tam tortille jednak
nie było to najlepsze danie jakie jadłem na tym wyjeździe.
...
nagrywałem tylko najciekawsze sceny. Dlatego teraz opiszę tylko te.
Jedną z takich bardzo ciekawych fragmentów był zjazd
między [Jakubkowem][wiki-jakubkowo] a [Mędrzycami][wiki-medrzyce].

Sam fakt tego, że jest tu szlak pieszy wiele mówi o ciekawym
ukształtowaniu terenu.

Kolejnym ciekawym fragmentem był zjazd do [Bogdanki][wiki-bogdanka]
TODO poprawić bo Bogdanka jest właśnie przed Mędrzycami

Zjazd do [Mędrzyc][wiki-medrzyce] też był bardzo miły, chociaż nie lekki.
...
chwilę prowadzić rower. Tutaj jest niebieski szlak pieszy.

Na niebie pojawiły się chmury. Sam dostałem alarm ostrzegawczy o możliwości
wystąpienia burz w tym miejscu.

Kolejnym zjazdem był do [Świecia nad Osą][wiki-swiecie-nad-osa].
...
dobić do 100km. Mieliśmy jechać do [Linowa][wiki-linowo] i dalej bezpośrednio
...
Ostatnim ciekawym miejscem była wieś [Szczupinki][wiki-szczuplinki], która
tak mentalnie również była "na uboczu".

Dotarliśmy do [Książek][wiki-ksiazki] i ponownie odwiedziliśmy tą samą
restaurację co wczoraj. Mieliśmy godzinę do odjazdu i 1km do 100km dlatego zaproponowałem
abyśmy pokręcili się po okolicy. Ostatecznie na moim GPS było 101km a na
Stravie 100.1km.
